...
def test_the_json_schema_version_is_pinned():
    """
    Версия формата меняется только осознанно.

    Правило записано у самой константы: версия растёт, когда меняется СМЫСЛ
    существующих ключей. Второй она стала, когда `name` у поставки перестал
    быть дословным текстом вендора — структура та же, но потребитель,
    сличавший имя с оглавлением .efd, перестал бы попадать, и молча.

    Проверка намеренно сличает с числом, а не с константой: сверка константы
    с самой собой пережила бы любую правку, в том числе забытую.
    """
    document = json.loads(format_json(plan(item()), source_count=1, elapsed=0.1))

    assert document["schema"] == 2, (
        "менять версию формата можно, но не мимоходом: допишите, что изменилось"
    )
