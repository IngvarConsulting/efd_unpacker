...
def move_to_trash(paths):
    """
    Переносит файлы в корзину. Возвращает перенесённые и не поддавшиеся.

    В корзину, а не насовсем: удаляются ЧУЖИЕ файлы, которые человек сам
    когда-то скачал, и ошибка в списке должна отменяться его же руками, без
    нашего участия.

    Своими силами это не делается: у каждой системы корзина своя, а у
    freedesktop она ещё и раскладывается по томам, с отдельным файлом
    сведений на каждый перенос. Send2Trash знает все три и весит полтысячи
    строк — писать это заново значило бы завести четвёртую реализацию
    спецификации, проверить которую можно только на одной системе из трёх.

    Отказ на одном файле не обрывает остальные: список пришёл из окна, и
    один недоступный файл не повод не убрать прочие.
    """
    ...
    for path in paths:
        try:
            send2trash(path)
            moved.append(path)
        except Exception as exc:  # noqa: BLE001 - причин отказа открытый набор
            logger.warning("Не удалось перенести в корзину %s: %s", path, exc)
            failed.append(path)
    return moved, failed
