...
UNAVAILABLE = "unavailable"  # не могу: нет программы, фильтр унёс всё
INSTALLED = "installed"      # не стал: каталог на месте, но можно настоять
RUNNING = "running"
...
#:
#: «Уже установлено» переключается, «нет программы» — нет, и разница между
#: ними не в тяжести, а в том, чьё это решение. Отсутствие программы для
#: .rar — невозможность: желание её не создаёт. А «каталог уже на месте» —
#: наше УМОЛЧАНИЕ, и человек знает про этот каталог то, чего не знаем мы:
#: распаковка могла оборваться на середине кнопкой «Остановить» или отказом,
#: и тогда внутри половина. Пока знак не переключался, единственным выходом
#: было удалить каталог в файловом менеджере.
#:
...
#: скопом — никогда. Иначе одно нажатие на списке из двадцати архивов
#: переписывало бы гигабайты, которые и так на диске, и человек узнавал бы
#: об этом по времени работы.
OFFERED = (PENDING, UNCHECKED, PARTIAL, FAILED)
...
class Mark(QAbstractButton):
    ...
    не срабатывает, так что готовую строку и «нет программы» не тронуть ни
    ...
    def paintEvent(self, _event) -> None:
        ...
        elif self._state in (UNCHECKED, INSTALLED):
            # Одинаково, и это намеренно. Для человека «уже установлено» —
            # такая же неотмеченная строка, которую можно отметить; разница
            # только в причине, а причина у всех пропусков живёт в нижнем
            # ярусе строки. Развести их цветом значило бы нарушить правило
            # «состояние кодируется формой, а не только цветом» ради
            # различия, из которого не следует разных действий.
