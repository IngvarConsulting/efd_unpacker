...
#:
#: 1 — первая редакция.
#: 2 — `name` у поставки больше не дословный текст вендора: с него снимается
#:     служебная обёртка «конфигурация "…"». Структура та же, но потребитель,
#:     сличавший имя с оглавлением .efd, перестал бы попадать молча — ровно
#:     тот случай, ради которого версия и существует.
JSON_SCHEMA = 2
