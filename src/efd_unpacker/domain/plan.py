...
def _distribution_item(result: Inspected, settings: PlanSettings) -> PlannedItem:
    ...
    # дистрибутив здесь будет объявлен распакованным: о полноте каталог не
    # говорит ничего. Отсюда это по-прежнему не решается — но и тупиком не
    # кончается: пропуск по ЭТОЙ причине окно показывает переключаемым
    # состоянием (rows.INSTALLED), и человек, знающий про свой каталог
    # больше нашего, возвращает строку в работу одним щелчком. Окно
    # передаёт настояние сюда через settings.is_installed.
    action, reason = Action.WRITE, None
