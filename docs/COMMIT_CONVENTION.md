...
## Скрытие из релиз ноутс

Если коммит нужен в истории, но не должен попадать в автоматически сгенерированные релиз ноутс
(например, временные работы по установке `Qt6`, `qt5-tools`, `lrelease`, диагностике CI и т.п.),
пометьте его одним из двух способов:
...
Предпочтительный вариант: trailer `Release-Notes: skip` в теле коммита.

Пример:

```text
build(windows): Обновлена временная диагностика поиска Qt SDK

Release-Notes: skip
```

Такой коммит останется в истории Git, но генератор релиз ноутс его пропустит.
