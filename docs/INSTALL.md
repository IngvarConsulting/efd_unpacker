...
- **macOS**: `.dmg` — два файла, под Apple Silicon и под Intel
- **Windows**: `setup.exe`
- **Linux**: `.AppImage`, `.deb`
...
Сборка под Intel запустится и на Apple Silicon через Rosetta 2, но медленнее — берите arm64, если он вам подходит.
...
После первого запуска команда `efd_unpacker` автоматически регистрируется в пользовательском `PATH`. Блок дописывается и в login-профиль (`~/.profile`, `~/.zprofile`, `~/.bash_profile`), и в rc-файл (`~/.bashrc`, `~/.zshrc`), поэтому команда появляется и в обычном окне терминала, а не только в login-сессии. В уже открытом терминале откройте новую вкладку. После удаления или перемещения `AppImage` новая shell-сессия больше не будет добавлять эту команду в `PATH`.
